...
# Choose engine vision model: ocr (default, Unlimited-OCR) or gemma (instruction-following VLM).
$OCR_MODEL = if ($env:OCR_MODEL) { $env:OCR_MODEL } else { "ocr" }
if ($OCR_MODEL -eq "ocr") {
...
function Test-Online { param([int]$Port) try { (Invoke-WebRequest -UseBasicParsing -TimeoutSec 2 "http://${HOST_IP}:${Port}/v1/models").StatusCode -eq 200 } catch { $false } }
...
if (Test-Online $OCR_PORT) {
  Write-Host "[ocr] engine already running on ${HOST_IP}:${OCR_PORT}."
  exit 0
}
Write-Host "[ocr] launching llama-server (Vulkan) on ${HOST_IP}:${OCR_PORT} ..."
$log = Join-Path $RUNTIME "server.log"
$p = Start-Process -FilePath $LLAMA_EXE -ArgumentList "-m",(Join-Path $MODEL_DIR $TEXT_Q),"--mmproj",(Join-Path $MODEL_DIR $PROJ_Q),"-ngl","999","-c","8192","--host",$HOST_IP,"--port",$OCR_PORT -RedirectStandardOutput $log -RedirectStandardError "$log.err" -WindowStyle Hidden -PassThru
